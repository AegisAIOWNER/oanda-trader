    ...
    def run_cycle(self):
        ...
        if best_signal:
            ...
            if ENABLE_AUTO_SCALE_UNITS:
                # Use auto-scaling position sizing
                logging.info(f"🔧 Using auto-scaling position sizing for {instrument}")
                
                units, risk_pct, sizing_info = self.position_sizer.calculate_auto_scaled_units(
                    ...
                    maximum_order_units=int(inst_metadata.get('maximumOrderUnits', 100000000)),
                    max_units_per_instrument=MAX_UNITS_PER_INSTRUMENT
                )
                
                # Check if trade should be skipped
                if units == 0:
                    skip_reason = sizing_info.get('skip_reason', 'Unknown reason')
                    logging.warning(f"⚠️  Skipping trade for {instrument}: {skip_reason}")
                    logging.info(f"   Sizing details: {sizing_info}")
                    
                    if self.structured_logger:
                        self.structured_logger.log_trade_decision(
                            instrument, signal, confidence, "SKIPPED",
                            f"Auto-scaling skip: {skip_reason}"
                        )
                    
                    # Store volatility reading even when trade is skipped
                    if self.enable_volatility_detection and self.db and volatility_info:
                        vol_data = {
                            'avg_atr': volatility_info['avg_atr'],
                            'state': volatility_info['state'],
                    ...
                    return True  # Continue but skip this trade
                
                # Log debug info for successful sizing
                logging.info(f"📊 Auto-scaling sizing details:")
                logging.info(f"   Units by margin: {sizing_info.get('units_by_margin', 'N/A')}")
                logging.info(f"   Units by risk: {sizing_info.get('units_by_risk', 'N/A')}")
                logging.info(f"   Final units: {sizing_info.get('final_units', units)}")
                logging.info(f"   Min trade size: {sizing_info.get('min_trade_size', 'N/A')}")
                logging.info(f"   Effective available margin: ${sizing_info.get('effective_available_margin', 'N/A'):.2f}")
                logging.info(f"   Trade value: ${sizing_info.get('trade_value', 'N/A'):.2f}")
                
            else:
                # Use legacy margin-based approach
